...
<head>
    ...
    <style type="text/css">
		...
		.the-icons li .active {background-color:#0088CC;color:#ffffff;}
		.fa-item.active {background-color:#0088CC;color:#ffffff;}
    </style>
    <script>
	    $(function(){
	    	$('#icons .fa-item').click(function(){
	    		$('#icons .fa-item').removeClass('active');
	    		$('#icons .fa-item i').removeClass('icon-white');
	    		$(this).addClass('active');
	    		$(this).children('i').addClass('icon-white');
				$('#icon').val($(this).children('i').attr('class').replace('icon-white', ''));
	    	});
	    	$('#icons .fa-item').each(function(){
				if ($(this).children('i').attr('class')==jQuery.trim('${fs_icon}')){
	    			$(this).click();
	    			return false;
	    		}
	    	});
	    	$('#icons .fa-item').dblclick(function(){
...
<body>
<!--请参考font-awesome!-->
<input type="hidden" id="icon" value="${fs_icon}" />
